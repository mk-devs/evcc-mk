...
func (wb *E3dc) Status() (api.ChargeStatus, error) {
	...
	data, err := ReadComplexTags(wb, []rscp.Message{*rscp.NewMessage(rscp.WB_REQ_EXTERN_DATA_ALL, nil)}, []rscp.Tag{rscp.WB_EXTERN_DATA}, cast.ToUint8SliceE)
...
func (wb *E3dc) TestWallboxTags(wb_idx uint8) error {
	...
	res3, err3 := ReadComplexTags(wb, []rscp.Message{*rscp.NewMessage(rscp.WB_REQ_STATUS, nil)}, []rscp.Tag{rscp.WB_STATUS}, cast.ToInt64E) // No info if plugged in!
	if err3 != nil {
		return err3
	}
	res3 = append(res3, -1)
